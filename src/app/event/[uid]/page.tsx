...
export default async function Page({ params }: { params: Promise<Params> }) {
  ...
  return (
    <Bounded className="py-10">
      <div className="grid grid-cols-1 items-center gap-10 pb-10 lg:grid-cols-2">
        ...
        <div className="text-white">
          <h1 className="font-display mb-4 border-b border-neutral-700 pb-2 text-4xl md:text-5xl">
            <PrismicText field={page.data.title} fallback="Even" />
          </h1>

          <div className="space-y-6">
            <p className="text-md font-semibold">Art</p>

            <PrismicRichText field={page.data.description} />

            <EventAttributes
              mood={page.data.mood}
              scentProfile={page.data.event_profile}
            />
      ...
      <OtherEvents currentEvenUid={uid} />
    </Bounded>
  );
}
...
export async function generateMetadata({
  ...
  return {
    ...
    description: `Discover ${asText(page.data.title)}, the newest event from Eclipse Privée.`,
    openGraph: {
