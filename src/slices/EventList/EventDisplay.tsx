...
export const EventDisplay = async ({ id }: EventDisplayProps) => {
  const client = createClient();
  const event = await client.getByID<Content.EventDocument>(id);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <EventAttributes
          scentProfile={event.data.event_profile}
          mood={event.data.mood}
          className="mb-10"
        />

        <div className="flex flex-wrap gap-4">
          <ButtonLink document={event} variant="Secondary">
            Discover
          </ButtonLink>
