...
/**
 * Props for `EventFeature`.
 */
export type EventFeatureProps = SliceComponentProps<Content.EventFeatureSlice>;

/**
 * Component for "EventFeature" Slices.
 */
const EventFeature: FC<EventFeatureProps> = async ({ slice }) => {
  const client = createClient();
  const event = isFilled.contentRelationship(slice.primary.event)
    ? await client.getByID<Content.EventDocument>(
        slice.primary.event.id,
      )
    : null;

  const formattedPrice = formatPrice(event?.data.price);
  return (
    <Bounded
        ...
        >
          ...
          <div className="flex justify-between p-10 pt-4">
            <div className="space-y-1">
              ...
              <p className="mt-2 text-gray-400">Eclipsed</p>
              <ButtonLink
                document={event}
                variant="Secondary"
                className="mt-6"
