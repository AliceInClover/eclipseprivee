import { createClient } from "@/prismicio";
import { formatPrice } from "@/utils/formatters";
import { PrismicNextImage } from "@prismicio/next";
import { PrismicText } from "@prismicio/react";
import { TransitionLink } from "./TransitionLink";

type OtherEventsProps = {
  currentEvenUid: string;
};

export const OtherEvents = async ({
  currentEvenUid,
}: OtherEventsProps) => {
  const client = createClient();
  const allEvens = await client.getAllByType("event");

  const otherEvents = allEvens.filter(
    (event) => event.uid !== currentEvenUid,
  );

  return (
    <div className="container mx-auto px-4">
      <h2 className="font-display mb-8 text-3xl text-white md:text-4xl">
        You may also like
      </h2>

      <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {otherEvents.map((event) => (
          <li key={event.id}>
            <TransitionLink document={event} className="group">
              <div className="relative aspect-square w-full transition-transform duration-500 group-hover:scale-105">
                <PrismicNextImage
                  field={event.data.bottle_image}
                  width={600}
                  height={600}
                  className="h-auto w-full"
                />
              </div>

              <div className="mt-8 space-y-1 text-white">
                <h3 className="font-display text-2xl">
                  <PrismicText field={event.data.title} />
                </h3>
                <p className="text-sm text-neutral-400">Eclipsed</p>
                <p className="text-base font-light">
                  {formatPrice(event.data.price)}
                </p>
              </div>
            </TransitionLink>
          </li>
        ))}
      </ul>
    </div>
  );
};
